...
public class SimLogger {
	public static enum TASK_STATUS {
		CREATED, UPLOADING, SUBMITTED, PROCESSING, DOWNLOADING, COMLETED, REJECTED_DUE_TO_VM_CAPACITY, REJECTED_DUE_TO_BANDWIDTH, UNFINISHED_DUE_TO_BANDWIDTH, UNFINISHED_DUE_TO_MOBILITY
	}
	...
	public void uploadStarted(int taskId, double taskUploadTime, double time) {
		taskMap.get(taskId).taskUploadStarted(taskUploadTime, time);
	}
	
	public void transferStarted(int taskId, double taskTransTime) {
		taskMap.get(taskId).taskTransferStarted(taskTransTime);
	}
	
	public void submitted(int taskId, double taskSubmissionTime) {
		taskMap.get(taskId).taskSubmitted(taskSubmissionTime);
	}
	...
	public void simPaused() throws IOException {
		...
		ETTmatrix = new ETTMatrix(numOfDataCenters, ETTdistributions);
	...
	private void createDistribution() {
		...
		for (Map.Entry<Integer, LogItem> entry : taskMap.entrySet()) {
			...
			if(value.getStatus() == SimLogger.TASK_STATUS.COMLETED) {
					...
					double processingTime = value.getEndTime()- value.getProcStartTime() - value.getDownloadTime();
...
class LogItem {
	...
	public void taskUploadStarted(double _taskUploadTime, double _time) {
		networkDelay += _taskUploadTime;
		taskUploadTime = _time;
		status = SimLogger.TASK_STATUS.UPLOADING;
	}
	public void taskTransferStarted(double _taskTransTime) {
		taskTransferTime = _taskTransTime;
	}
	
	public void taskSubmitted(double time) {
		status = SimLogger.TASK_STATUS.SUBMITTED;
		taskSubmissionTime = time;
	}
	...
	public void taskDownloadStarted(double _taskDownloadTime) {
		networkDelay += _taskDownloadTime;
		taskDownloadTime = _taskDownloadTime;

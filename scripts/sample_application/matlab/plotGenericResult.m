function [] = plotGenericResult(rowOfset, columnOfset, yLabel, appType, calculatePercentage)
    ...
    all_results = zeros(numOfSimulations, size(scenarioType,1), numOfMobileDevices);
        ...
        for i=1:size(scenarioType, 1)
            for j=1:numOfMobileDevices
                try
                    ...
                    filePath = strcat(folderPath,'/ite',int2str(s),'/SIMRESULT_',char(scenarioType(i)),'_SJF_',int2str(mobileDeviceNumber),'DEVICES_',appType,'_GENERIC.log');
                    readData = dlmread(filePath);
